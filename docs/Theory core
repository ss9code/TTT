...
$P$ は「存在の完成状態」を表します。右辺の6つの項はそれぞれ独立した次元を持ち、それらの総和として $P$ が形成されます。この方程式は静的な座標ではなく、**「崩壊（物理の宿命）と創造（精神の抵抗）が絶え間なくせめぎ合う動的なプロセス」**を表す方程式です。
...
- *司法・社会構造への応用 → [judicial_theory.md](./judicial_theory.md)*
